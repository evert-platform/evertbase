...
class PluginIDs(db.Model):
    plugin_ID = db.Column('id', db.Text, primary_key=True)  # Should this text be the plugin folder name?
    # I'm unsure what else needs to be in this table?


class PluginTimeData(db.Model):
    # This table needs:
    # - original data ID
    # - generated data ID
    # - generated data
    # - should this contain the timestamp? (Depends on how the plotting works)

    data_id = db.Column('id', db.Text, primary_key=True)
    origin_id = db.Column('origin_id', db.ForeignKey('MeasurementData.id', ondelete="CASCADE", onupdate="CASCADE"))
    tag = db.Column('tag', db.ForeignKey('tags.id', ondelete="CASCADE", onupdate="CASCADE"))
    tag_value = db.Column('tag_value', db.Float)
    plugin_tag = db.Column('plugin_tag', db.ForeignKey('PluginIDs.id', ondelete="CASCADE", onupdate="CASCADE"))

    # Should this contain the same @staticmethods as MeasurementData?
...
class PluginFeaturesData(db.Model):
    ...
    tag = db.Column('tag', db.ForeignKey('tags.id', ondelete="CASCADE", onupdate="CASCADE"))
    timestamp = db.Column('timestamp', db.Text)
    feature_value = db.Column('feature_value', db.Float)
    plugin_tag = db.Column('plugin_tag', db.ForeignKey('PluginIDs.id', ondelete="CASCADE", onupdate="CASCADE"))
